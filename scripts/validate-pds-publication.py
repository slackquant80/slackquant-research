...
def main()->int:
    ...
    for tok in ['fixed 25% F2R / 75% ADAA','current asset-level target','the Core is not reset to 25/75 each day','Current asset-level target and live allocation state beyond the disclosed fixed Core policy','/systems/pds/dashboard/','/systems/adaa/','/systems/f2r/','pdsPublicSnapshot','Public / delayed / private','PDS is provider-agnostic and is not defined by any particular pair of strategies','Portfolio Integration & Allocation','const recentReturnRows =','Recent 12-Month Released Returns','PDS + Dynamic FX (5bp)','historical comparison series only','<th>PDS + Dynamic FX (5bp)</th><th>PDS Core</th><th>F2R</th><th>ADAA</th>','/resources/systems/pds/PDS_System_Documentation_v1.0.pdf','System Documentation ↗']:require(pds,tok,'PDS page')
    ...
    for tok in ['PDS_PUBLIC_DATA','Public dashboard','Overview','Core Performance','Portfolio History','Investor / FX','System & Disclosure','KRW Investor View','F2R 25% · ADAA 75%','Current asset targets','Monthly execution only · daily weights drift','<div class="brand-name">SlackQuant</div><div class="brand-sub">Systems</div>','pds-product-kicker','href="/systems/pds/"','href="/systems/adaa/"','href="/systems/f2r/"','Recent 12-Month Returns','PDS + Dynamic FX (5bp)','Completed delayed returns · KRW investor view','portfolio-return-table','overflow-x:auto!important','public-table.portfolio-return-table','min-width:650px!important',"annualRows(['DYNAMIC_COSTED','PDS_ACTIVE_CORE','F2R','ADAA','SPY_AGG_60_40'])",'public-history-toggle','font-size:9.5px!important','Hide history']:require(public_html,tok,'PDS standalone public dashboard',casefold=True)

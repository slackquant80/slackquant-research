...
class PageParser(HTMLParser):
    def __init__(self):
        super().__init__(convert_charrefs=True)
        self.ids=set(); self.in_toc=False; self.toc_targets=[]; self.kicker_depth=0; self.kickers=[]; self.kbuf=[]; self.hrefs=[]
    def handle_starttag(self,tag,attrs):
        a=dict(attrs)
...
def browser_ids(text:str)->list[str]:
    m=re.search(r'<script id="sq-method-browser-script">(.*?)</script>',text,re.S)
    if not m:return []
    return re.findall(r'\{\s*id:\s*"(QM\d{3})"',m.group(1))
...
def check(root:Path,label:str):
    ...
    leaked=sorted(root.rglob('*.qmd'))
    if leaked: fail(f'{label}_qmd_source_leak count={len(leaked)} example={leaked[0].relative_to(root)}')
    idx=root/'index.html'
    ...
    if ip.global_rows!=26 or ip.catalog_rows!=26: fail(f'{label}_rows global={ip.global_rows} catalog={ip.catalog_rows}')
    ids=[]; cats=[]
    for m in ip.metas:
    ...
    labs=sorted(root.rglob('labs/index.html'))
    if len(articles)!=26: fail(f'{label}_articles={len(articles)}')
    if len(labs)!=26: fail(f'{label}_labs={len(labs)}')

    pages=articles+labs
    for p in pages:
        text=p.read_text(encoding='utf-8',errors='replace')
        bids=browser_ids(text)
        if len(bids)!=26 or set(bids)!=EXPECTED_IDS:
            fail(f'{label}_browser_registry={p.relative_to(root)} count={len(bids)}')
        m=re.search(r'QM\d{3}',str(p))
        if m and m.group(0) not in bids:
            fail(f'{label}_browser_current_missing={p.relative_to(root)}:{m.group(0)}')
    ...
    for p in articles:
        ...
        if not any('labs/index.html' in h for h in pp.hrefs):
            fail(f'{label}_article_lab_link_missing={p.relative_to(root)}')

    print(f'METHODS_UI_INTEGRITY_{label.upper()}_PASS catalog_rows=26 articles=26 labs=26 categories=7 browser_registry=26x52 article_lab_links=26 toc_targets=valid qmd_source_leaks=0')
